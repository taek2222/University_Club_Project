...
const LocationButton = ({ locationImage }) => {
  ...
  return (
    <>
      ...
      {isModalOpen && (
        <div
          className="fixed inset-0 pr-3 bg-black bg-opacity-0"
          onClick={closeModal}
        >
          <div className="relative top-1/2 transform -translate-y-1/2 mx-auto p-4 border shadow-lg rounded-md bg-white max-w-sm md:max-w-md lg:max-w-lg">
            <div className="flex pb-3 items-center justify-between">
              <div className="flex items-center">
                {/* 위치 모달 제목 (아이콘) */}
                <img src={iconUrl} alt="Club Icon" className="w-7 h-7" />
              ...
              <button
                onClick={closeModal}
                className="text-black font-bold rounded hover:bg-gray-100"
              >
                닫기
              </button>
            </div>
